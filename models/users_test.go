...
func testUserToManyNotes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, noteDBTypes, false, noteColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, noteDBTypes, false, noteColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadNotes(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Notes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Notes = nil
	if err = a.L.LoadNotes(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Notes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpNotes(t *testing.T) {
	var err error
	...
	foreigners := []*Note{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, noteDBTypes, false, strmangle.SetComplement(notePrimaryKeyColumns, noteColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Note{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddNotes(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Notes[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Notes[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Notes().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpNotes(t *testing.T) {
	var err error
	...
	err = a.SetNotes(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetNotes(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyRemoveOpNotes(t *testing.T) {
	var err error
	...
	err = a.AddNotes(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveNotes(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
