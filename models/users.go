...
func (userL) LoadNotes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
...
// SetNotes removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's Notes accordingly.
// Replaces o.R.Notes with related.
// Sets related.R.User's Notes accordingly.
func (o *User) SetNotes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Note) error {
	query := "update \"notes\" set \"user_id\" = null where \"user_id\" = $1"
	...
	return o.AddNotes(ctx, exec, insert, related...)
}

// RemoveNotes relationships from objects passed in.
// Removes related items from R.Notes (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveNotes(ctx context.Context, exec boil.ContextExecutor, related ...*Note) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.Notes {
			...
			ln := len(o.R.Notes)
			if ln > 1 && i < ln-1 {
				o.R.Notes[i] = o.R.Notes[ln-1]
			}
			o.R.Notes = o.R.Notes[:ln-1]
			break
		}
	}
