...
func testNoteToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, noteDBTypes, true, noteColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Note struct: %s", err)
	}
	...
	slice := NoteSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Note)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testNoteToOneSetOpUserUsingUser(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, noteDBTypes, false, strmangle.SetComplement(notePrimaryKeyColumns, noteColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
...
func testNoteToOneRemoveOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	noteDBTypes = map[string]string{`ID`: `integer`, `UserID`: `integer`, `FirstName`: `text`, `LastName`: `text`, `Title`: `text`, `Note`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
